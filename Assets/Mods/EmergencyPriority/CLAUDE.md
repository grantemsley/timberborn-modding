...
| 2 | `BuilderHubWorkplaceBehavior.Decide` | Prefix | Iterate emergency jobs whose district matches this hub's district. The comparison uses `DistrictBuilding.GetDistrictOrConstructionDistrict()` (NOT `.District` directly) because unfinished construction sites have `District = null` and instead use `ConstructionDistrict`. For each matching job, call `ConstructionJob.StartConstructionJob(agent, accessible)`. First accepting job short-circuits the original `Priorities.Descending` loop. If at least one same-district emergency exists but none can accept this beaver (build slot full, no materials needed, etc.), return `Decision.ReleaseNow()` instead of falling through — that prevents extra builders from picking up a normal-priority construction while the emergency is unfinished. If no emergency is in this hub's district, fall through to vanilla. Reads `_accessible` via reflection. |
