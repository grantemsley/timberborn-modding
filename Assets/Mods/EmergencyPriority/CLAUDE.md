...
| 7 | `CarryRootBehavior.Decide` | Prefix | For emergency-employed builders, return `Decision.ReleaseNow()` to skip *new* haul pickups — the beaver falls through the behavior tree to `WorkerRootBehavior` and picks up the emergency. Acts as a backstop for `LaborBehavior` decorators on the hub that might reserve a haul while emergency is active. Exception: if the beaver is already carrying cargo (`GoodCarrier.IsCarrying`), let vanilla finish the delivery to wherever they were headed (the original destination, or fallback inventory). Reason: a beaver hauling to emergency A whose emergency flag is then cleared shouldn't carry the cargo through to emergency B — it looks weird and wastes the in-flight delivery. Letting vanilla complete drops cargo cleanly, then re-decide picks up the new emergency. In-progress hauls that the user explicitly wants interrupted are handled by `EmergencyInterruptionService` at the moment Emergency is set, which drops cargo on the ground via `RecoveredGoodStackSpawner`. |
