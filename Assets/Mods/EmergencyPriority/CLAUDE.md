...
| 7 | `CarryRootBehavior.Decide` | Prefix | For emergency-employed builders with the registry holding any emergency, skip picking up *new non-emergency* hauls. Two exceptions let vanilla run: (a) `GoodCarrier.IsCarrying == true` — already carrying, let vanilla finish the delivery (the G2 fix; aborting in-flight looks weird and wastes goods); (b) the reservation's destination inventory is on an emergency construction site (checked by `GoodReserver.CapacityReservation.Inventory.GetComponent<ConstructionJob>()` being in the registry) — this is the "fetching materials FOR the emergency" path that `StartConstructionJob → StartNeededMaterialsDelivery` initiates, and blocking it would leave bots/beavers stuck idle next to a missing-materials emergency. In-progress hauls the user explicitly wants interrupted are handled by `EmergencyInterruptionService` at the moment Emergency is set (drops cargo via `RecoveredGoodStackSpawner`). |
